 ...
 * Global
 */
sa_builder_t* _global_sa_builder;
...
void* sa_builder_store_suffixes_thread(const uint8_t thread_id) {
  const uint64_t text_length = dna_text_get_length(_global_sa_builder->enc_text);
  const uint8_t* const enc_text = dna_text_get_text(_global_sa_builder->enc_text);
  ...
  const uint64_t extended_text_length = dna_text_get_length(_global_sa_builder->enc_text)+SA_BWT_CYCLIC_LENGTH;
  const uint64_t* const kmer_count = _global_sa_builder->kmer_count;
  sa_group_t* const sa_groups = _global_sa_builder->sa_groups;
  for (sa_pos=0;i<extended_text_length;++i,++sa_pos,++tp) {
    ...
    if (thread_id==0 && tp==SA_BUILDER_STORE_SUFFIXES_TICKER_STEP) {
      ticker_update(&_global_sa_builder->ticker,tp); tp = 0;
    }
  }
  // Return
  return NULL;
...
void sa_builder_store_suffixes(
    sa_builder_t* const sa_builder,
    const bool verbose) {
  _global_sa_builder = sa_builder; // Set global data
